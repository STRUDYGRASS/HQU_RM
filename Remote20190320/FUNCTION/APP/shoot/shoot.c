...
//模仿RTOS时钟
void shoot_clock_init(void);
//#define CLOCK_TIME 50000 移动到main函数中
...
void shoot_init(void)
{
    ...
    ramp_init(&trigger_motor.fric1_ramp, SHOOT_CONTROL_TIME * 0.01f, Fric_DOWN, Fric_OFF);
    ramp_init(&trigger_motor.fric2_ramp, SHOOT_CONTROL_TIME * 0.01f, Fric_DOWN, Fric_OFF);

    trigger_motor.ecd_count = 0;
    trigger_motor.angle =  TREncoder.raw_value * Motor_ECD_TO_ANGLE;
...
//鼠标右键按下加速摩擦轮，使得左键低速射击， 右键高速射击
        ...
        shoot_CAN_Set_Current = trigger_motor.given_current;
				//此处为了使用以前的调用结构，将得到的解析值直接传给TriggerMotot_Out()函数，将函数直接写入其中
				Set_CloudMotor_Current(CloudMotor_Out()[0],CloudMotor_Out()[1],shoot_CAN_Set_Current);
...
static void Shoot_Feedback_Update(void)
{
    ...
    speed_fliter_3 = speed_fliter_2 * fliter_num[0] + speed_fliter_1 * fliter_num[1] + (TREncoder.rpm * Motor_RMP_TO_SPEED) * fliter_num[2];
    trigger_motor.speed = speed_fliter_3;

    
//电机圈数重置， 因为输出轴旋转一圈， 电机轴旋转 36圈，将电机轴数据处理成输出轴数据，用于控制输出轴角度
    if (TREncoder.raw_value - TREncoder.last_raw_value > Half_ecd_range)
    {
        trigger_motor.ecd_count--;
    }
    else if (TREncoder.raw_value - TREncoder.last_raw_value < -Half_ecd_range)
    {
        trigger_motor.ecd_count++;
    }
    ...
    //计算输出轴角度
    trigger_motor.angle = (trigger_motor.ecd_count * ecd_range + TREncoder.raw_value) * Motor_ECD_TO_ANGLE;
    //微动开关
...
void TIM1_DAC_IRQHandler(void)
{
    if (TIM_GetITStatus(TIM1,TIM_IT_Update)!= RESET)
    {
        TIM_ClearITPendingBit(TIM1,TIM_IT_Update);
        TIM_ClearFlag(TIM1, TIM_FLAG_Update);
        count = (count++) % CLOCK_TIME;
    }
}*/
